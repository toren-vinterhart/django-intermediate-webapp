...
class RegistrationApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'from@example.com', to=[email])
...
class ActivationApiView(APIView):
    def get(self, request, token, *args, **kwargs):
        try:
            token = jwt.decode(token, config('SECRET_KEY'), algorithms=["HS256"]) # used decouple to get SECRET_KEY
...
class ActivationResendApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'from@example.com', to=[user_obj.email])
        EmailThread(email_obj).start()
