...
class PostSerializer(serializers.ModelSerializer):
    ...
    snippet = serializers.ReadOnlyField(
        source="get_snippet"
    )  # get_snippet is a method in Post model
    relative_url = serializers.URLField(
        source="get_absolute_api_url", read_only=True
    )  # get_absolute_api_url is a method in Post model
    absolute_url = serializers.SerializerMethodField(
        method_name="get_abs_url"
    )  # method name must be get_{field_name} e.g. get_absolute_url if we don't set method_name
